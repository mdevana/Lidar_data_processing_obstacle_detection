...
void proximity(int n,const std::vector<std::vector<float>> points,std::vector<bool>& track_pro_pts, KdTree* tree,std::vector<int>& cluster_one, float distanceTol){
	...
	std::vector<int> nearby_pts = tree->search(points[n],distanceTol);

	for(int id : nearby_pts){

		if(!track_pro_pts[id]) {

			proximity(id,points,track_pro_pts,tree,cluster_one,distanceTol);
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>> points, KdTree* tree, float distanceTol)
{
	...
	std::vector<bool> track_processed(points.size(),false);
	...
	for(int i = 0;i<points.size();i++){
		...
		proximity(i,points,track_processed,tree,single_cluster,distanceTol);

		/*for(int index : single_cluster)
         std::cout << index << ",";
  	    std::cout << std::endl;*/
...
int main ()
{
	...
	std::vector<int> nearby = tree->search({0.2,-7.0},3.0);
